...
public class OrderItemController {
	...
	public String process(@PathVariable(name = "orderIdx") int orderIdx, Model model, HttpSession session) {
		...
		if (result.isPresent()) {
			...
			System.out.println("모델 리스트 값" + model);
		}
	...
	public String detail(@PathVariable(name = "orderIdx") int orderIdx, Model model, HttpSession session) {
		...
		if (result.isPresent()) {
			...
			List<OrderDetail> detailList = order.getDetailList();
			model.addAttribute("dto", order);
			System.out.println("dto의 정보를 다 가져왔다:" + order);
			model.addAttribute("detailList", detailList);
	...
	@Transactional
	@PostMapping("insert")
	public String insert(OrderItemDTO dto, @RequestParam(name = "productCode", defaultValue="0") int productCode,
						 @RequestParam(name = "amount", defaultValue="0") int amount,
